...
namespace POT_SEM.Services.Translation
{
    ...
    public class DatabaseTranslationService : ITranslationStrategy
    {
        ...
        public async Task<string?> TranslateWordAsync(string word, string sourceLang, string targetLang)
        {
            ...
            {
                Console.WriteLine($"❌ Database lookup failed for '{word}': {ex.Message}");
                return null;
            }
        }
            ...
            string targetLang)
        {
            ...
            {
                Console.WriteLine($"❌ Database batch lookup failed: {ex.Message}");
            }
            ...
            string? furigana = null)
        {
            try
            {
                ...
                Console.WriteLine($"💾 Saved translation: '{originalWord}' → '{translation}'");
            }
            catch (Exception ex)
            {
                ...
                {
                    Console.WriteLine($"⚠️ Failed to save translation: {ex.Message}");
                }
            }
        }
        ...
        private async Task IncrementUsageAsync(long translationId)  // ← ZMENA: int → long
        {
            ...
            {
                Console.WriteLine($"⚠️ Failed to increment usage: {ex.Message}");
            }
        }
    }
}